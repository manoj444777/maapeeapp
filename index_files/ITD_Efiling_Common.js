...
function backToHomePageGuest() {
    ...
    document.forms[0].action = "https://www.incometaxindiaefiling.gov.in/home/";
    document.forms[0].onsubmit = "";
    document.forms[0].submit();
}
...
function getStaticPageFromHomePage(staticContentsUrl) {
    ...
    loadStaticPageMme("staticContentsUrl", "https://www.incometaxindiaefiling.gov.in/eFiling/Portal/" + staticContentsUrl + "?" + Math.random());

    document.getElementById("staticContentsUrl").className = 'leftColumn';

    loadStaticPageMme("ltcol", "https://www.incometaxindiaefiling.gov.in/eFiling/Portal/Prelogin_Left_Column.html?" + Math.random());
    loadStaticPageMme("header", "https://www.incometaxindiaefiling.gov.in/eFiling/Portal/Header.html?" + Math.random());
...
function downloadThisForm(formName, formType) {
    ...
    if (formType == 'excelZip' || formType == 'excel' && (asstYear == '2017')) {
        ...
        window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/" + asstYear + '/' + formName.replace("-", "") + '_' + asstYear + '.zip?' + Math.random(), '_blank');

    } else if (formType == 'pdf') {
        window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/" + asstYear + '/Form-' + formName + '-' + asstYear + '.pdf?' + Math.random(), '_blank');
    } else if (formType == 'jfx') {
        if (formName == 'CA') {
            window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/" + asstYear + '/FORMS_' + asstYear + '.zip?' + Math.random(), '_blank');
        } else {
            window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/FORM_UTILITY/" + formName + '_' + asstYear + '.zip?' + Math.random(), '_blank');
        }
        ...
        window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/ITR_UTILITY/" + asstYear + '/' + formName + '_' + asstYear + '.zip?' + Math.random(), '_blank');
    } else if (formType == 'jfxITR' && !(asstYear == '2015' || asstYear == '2014')) {
...
function downloadThisFormDTAA(formName, formType) {
    ...
    if (formType == 'excelZip' || formType == 'excel') {
        window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/2015/DTAA/" + formName.replace("-", "") + '_' + "2015" + '.zip?' + Math.random(), '_blank');

    } else if (formType == 'jfxITR') {
        alert('ITR utilities are developed using JAVA technology and effort has been made to make it user friendly, simpler and faster preparation of tax returns. This utility can run on operating systems like Windows 7.0 or above, Latest Linux and Mac OS 10.10(OS X Yosemite), Where Java Runtime Environment Version 7 update 6 or above is installed.\n\nThe downloaded ZIP folder of the requisite ITR form should be Extracted/Unzipped before opening the JAVA utility.');
        window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/ITR_UTILITY/2015/DTAA/" + formName + '_' + "2015" + '.zip?' + Math.random(), '_blank');
    }
...
function disableForms() {
    ...
    try {
        ...
        if (asYear == '2017') {
            ...
            document.getElementById("itr6xslId").style.display = "";
            document.getElementById("itr7xslId").style.display = "";
            document.getElementById("javaHeader").innerHTML = "<img alt=\"Java Utility\" src=\"https://www.incometaxindiaefiling.gov.in/itax/images/java.gif\"/> Java Utility";
            showHideCol(document.getElementById('xslHeader').parentNode.parentNode.parentNode, 2, '');
            showHideCol(document.getElementById('xslHeader').parentNode.parentNode.parentNode, 3, '');
            document.getElementById("DTAAtable").style.display = "none";
...
function getStaticPageHindi(staticContentsUrl) {
    ...
    loadStaticPageMme("staticContentsUrl", "https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/" + staticContentsUrl + "?" + Math.random());
...
function getStaticHindiPageFromHomePageOnlyHindi(staticContentsUrl) {
    ...
    loadStaticPageMme("ltcol", "https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/Prelogin_Left_Column.html?" + Math.random());
    loadStaticPageMme("header", "https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/Header.html?" + Math.random());
...
function setStaticContents(staticContentsUrl, id) {
    ...
    staticContentsUrl = "https://www.incometaxindiaefiling.gov.in/eFiling/Portal/" + staticContentsUrl;
        ...
        buttons: {
            /*  "Continue to NSDL website": function() {
		          ...
		          "Login to e-Filing account": function() {
		            window.open('https://portal.incometaxindiaefiling.gov.in/e-Filing/UserLogin/LoginHome.html',"_self");
		          },*/
            Register: function () {
                window.open('https://portal.incometaxindiaefiling.gov.in/e-Filing/Registration/RegistrationHome.html', "_blank");
            }
        }
    });
}
...
function openDisclaimer2(path) {
    ...
    $("#disclaimerCheckRefund").dialog({
        ...
        buttons: {
            ...
            "Login to e-Filing account": function () {
                window.open('https://portal.incometaxindiaefiling.gov.in/e-Filing/UserLogin/LoginHome.html?nextPage=myReturn', "_self");
            },
            Cancel: function () {
                $(this).dialog("close");
            }
...
function changeCSS(cssFile, cssLinkIndex) {
    ...
    newlink.setAttribute("href", "https://www.incometaxindiaefiling.gov.in/itax/styles/" + cssFile);
...
function changeImageToContrast() {
    ...
    for (var i = 0; i < imgArray.length; i++) {
        ...
        img.src = 'https://www.incometaxindiaefiling.gov.in' + newSrc;
    }
...
function onlyNumbersDotComma(e) {
    var keynum;
    ...
    $(document).ready(function () {
        ...
        $(".help").hover(function () {
            if ($(".helpmenu.hindiHelp").length == 0) {
                $(".helpmenu").load("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/headerHelp.html");
            } else {
                $(".helpmenu.hindiHelp").load("https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/headerHelp.html");
            }
        }, function () {
            return;
        });
...
function getOffset(el) {
    ...
    return {
        top: _y - $(document).scrollTop(),
        left: _x - $(document).scrollLeft()
    };
}
...
function downloadDSCUtility() {
    ...
    window.open("https://www.incometaxindiaefiling.gov.in/eFiling/Portal/DownloadUtil/DSC_UTILITY/DSC_UTILITY.zip?" + Math.random(), '_blank');
}
...
function goToPageHindi(staticContentsUrl, page, flag) {
    ...
    loadStaticPageMme("ltcol", "https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/Prelogin_Left_Column.html?" + Math.random());
    loadStaticPageMme("header", "https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/Header.html?" + Math.random());
    staticContentsUrl = "https://www.incometaxindiaefiling.gov.in/eFiling/PortalHindi/" + staticContentsUrl;
    $("#staticContentsUrl").load(staticContentsUrl, function (responseTxt, statusTxt, xhr) {
        showPage(page);
        if (flag) {
            var asYear = document.getElementById("asstYear").value;
...
function checkAlphabetApostrophe(evt) {
    var keynum;
    if (evt.keyCode > 0) {
        keynum = evt.keyCode;
...
function onKeyPressAlphabetsAndSpaceOnly(evt) {
    var keynum;
    if (evt.keyCode > 0) {
        keynum = evt.keyCode;
    } else if (typeof (evt.charCode) != "undefined") {
...
function checkAlphabetApostrophe(evt) {
    ...
    if (!(keynum >= 65 && keynum <= 90) && !(keynum >= 97 && keynum <= 122) &&
        !(keynum == 39) && (keynum != 8) && (keynum != 46) && (keynum != 9)) {
        return false;
    } else {
        return true;
    }
...
function onKeyPressAlphabetsAndSpaceOnly(evt) {
    ...
    if (!(keynum >= 65 && keynum <= 90) && !(keynum >= 97 && keynum <= 122) &&
        (keynum != 32) && (keynum != 8) && (keynum != 46) && (keynum != 9)) {
        return false;
    } else {
        return true;
    }
